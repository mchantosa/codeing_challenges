...
class Matrix {
    ...
    isValid(i, j) {
      return this.storage[i] !== undefined && this.storage[i][j] !== undefined;
    }
    ...
    slice(rowRange, colRange) {
      if(rowRange[1] > this.m || colRange[1] > this.n) return this;
  
      const result = [];
      for(let i = rowRange[0]; i < rowRange[1]; i++) {
        result.push(this.storage[i].slice(colRange[0], colRange[1]))
      }
  
      const resultMatrix = new Matrix(0,0);
      resultMatrix.initialize(result);
      return resultMatrix;
    }
  
    transpose() {
      const returnMatrix = new Matrix(this.n, this.m);
      for(let i = 0; i < this.n; i++) {
        for(let j = 0; j < this.m; j++) {
          returnMatrix.insert(i, j, this.storage[j][i])
        }
      }
      return returnMatrix;
    }
    ...
    multiply(matrix) {
      if(this.n !== matrix.m) return null;
      ...
      let matrixCols = matrix.transpose()
      for(let i = 0; i < this.m; i++) {
        for(let j = 0; j < matrix.n; j++) {
          result.insert(i, j, dotProduct(this.storage[i], matrixCols.storage[j]))
        }
      }
