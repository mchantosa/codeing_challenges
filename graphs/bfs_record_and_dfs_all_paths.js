...
function bfs(vertex){
  ...
  for (let i = 0; i < queue.length; i++) {
    ...
    for (let j = 0; j < current.edges.length; j++) {
      const neighbor = current.edges[j];
      if (!queue.includes(neighbor)) {
        queue.push(neighbor);
      }
    }
  }
  return queue.map(vertex => vertex.id);
}
  ...
  function traverse(current, path) {
   //base cases
    if (current.id === destination) {
      paths.push(path);
      return;
    } else if (visited.has(current.id)) {
      return;
    } 
   ...
   //recursive cases
    ...
    for (let i = 0; i < current.edges.length; i++) {
      const neighbor = current.edges[i];
      path.push(neighbor.id)
      traverse(neighbor, path.slice());
    }
